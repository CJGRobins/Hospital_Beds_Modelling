...
total_dch_ne_ts_arima_list <- total_dch_ne_ts %>%
  group_by(Diagnosis_Group) %>%
  nest()

total_dch_ne_ts_arima_7 <- total_dch_ne_ts_arima_list %>%
...
# Generating Model Fit (arima) and Tidying --------------------------------------------


total_dch_ne_ts_arima_fit <-
  total_dch_ne_ts_arima_7 %>%
  mutate(fit.arima = map(data_train_7.ts, auto.arima))

total_dch_ne_ts_arima_forecast <-
  total_dch_ne_ts_arima_fit %>%
  mutate(forecast.arima = map(fit.arima, forecast, h = 60))

total_dch_ne_ts_arima_forecast_tidy <-
  total_dch_ne_ts_arima_forecast %>%
  mutate(sweep = map(
    forecast.arima,
    sw_sweep,
...
total_dch_ne_ts_arima_forecast_tidy$`Occupied Beds`[total_dch_ne_ts_arima_forecast_tidy$`Occupied Beds` <
                                                      0] <-
  0
total_dch_ne_ts_arima_forecast_tidy$lo.80[total_dch_ne_ts_arima_forecast_tidy$lo.80 <
                                            0] <- 0
total_dch_ne_ts_arima_forecast_tidy$lo.95 [total_dch_ne_ts_arima_forecast_tidy$lo.95 <
                                             0] <- 0
