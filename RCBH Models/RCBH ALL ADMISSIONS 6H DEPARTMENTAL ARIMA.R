...
total_ts_dept_auto.arima_list <- total_ts_dept %>%
  select((-new_date)) %>%
  group_by(Specialty) %>%
  nest()

total_ts_dept_auto.arima_7 <- total_ts_dept_auto.arima_list %>%
...
total_ts_dept_auto.arima_fit <-
  total_ts_dept_auto.arima_7 %>%
  mutate(fit.auto_arima = map(data_train_7_ts, auto.arima))

total_ts_dept_auto.arima_forecast <-
  total_ts_dept_auto.arima_fit %>%
  mutate(forecast.auto_arima = map(fit.auto_arima, forecast, h = 240))

total_ts_dept_auto.arima_forecast_tidy <-
  total_ts_dept_auto.arima_forecast %>%
  mutate(sweep = map(
    forecast.auto_arima,
    sw_sweep,
...
total_ts_dept_auto.arima_forecast_tidy$`Occupied Beds`[total_ts_dept_auto.arima_forecast_tidy$`Occupied Beds` <
                                                    0] <-
  0
total_ts_dept_auto.arima_forecast_tidy$lo.80[total_ts_dept_auto.arima_forecast_tidy$lo.80 <
                                          0] <- 0
total_ts_dept_auto.arima_forecast_tidy$lo.95 [total_ts_dept_auto.arima_forecast_tidy$lo.95 <
                                           0] <- 0

total_ts_dept_auto.arima_forecast_tidy$date_value <-
  rep(seq.POSIXt(
    from = total_ts$new_date[1],
    by = "6 hours",
    length.out = nrow(total_ts_dept_auto.arima_forecast_tidy)/n_groups(total_ts_dept_auto.arima_forecast_tidy)
