...
total_ts_ne_dept_auto.arima_list <- total_ts_ne_dept %>%
  select((-new_date)) %>%
  group_by(Specialty) %>%
  nest()

total_ts_ne_dept_auto.arima_7 <- total_ts_ne_dept_auto.arima_list %>%
  mutate(data_train_7_ts = map(
    .x = data,
    .f = ts,
    frequency = 28
  ))
...
total_ts_ne_dept_auto.arima_fit <-
  total_ts_ne_dept_auto.arima_7 %>%
  mutate(fit.auto_arima = map(data_train_7_ts, auto.arima))

total_ts_ne_dept_auto.arima_forecast <-
  total_ts_ne_dept_auto.arima_fit %>%
  mutate(forecast.auto_arima = map(fit.auto_arima, forecast, h = 240))

total_ts_ne_dept_auto.arima_forecast_tidy <-
  total_ts_ne_dept_auto.arima_forecast %>%
  mutate(sweep = map(
    forecast.auto_arima,
    sw_sweep,
...
total_ts_ne_dept_auto.arima_forecast_tidy$`Occupied Beds`[total_ts_ne_dept_auto.arima_forecast_tidy$`Occupied Beds` <
                                                         0] <-
  0
total_ts_ne_dept_auto.arima_forecast_tidy$lo.80[total_ts_ne_dept_auto.arima_forecast_tidy$lo.80 <
                                               0] <- 0
total_ts_ne_dept_auto.arima_forecast_tidy$lo.95 [total_ts_ne_dept_auto.arima_forecast_tidy$lo.95 <
                                                0] <- 0

total_ts_ne_dept_auto.arima_forecast_tidy$date_value <-
  rep(seq.POSIXt(
    from = total_ts_ne$new_date[1],
    by = "6 hours",
    length.out = nrow(total_ts_ne_dept_auto.arima_forecast_tidy)/n_groups(total_ts_ne_dept_auto.arima_forecast_tidy)
  ), times = n_groups(total_ts_ne_dept_auto.arima_forecast_tidy))

total_ts_ne_dept_arima_forecast_export <- total_ts_ne_dept_auto.arima_forecast_tidy %>%
  select(Specialty, key, `Occupied Beds`, lo.80, lo.95, hi.80, hi.95)

write.csv(total_ts_ne_dept_arima_forecast_export, "ne_6h_arima.csv")
