...
admissions_daily_ne$diagnosis_1 <- as.character(admissions_daily_ne$diagnosis_1)
admissions_daily_ne$`ICD10 Short` <- as.character(admissions_daily_ne$`ICD10 Short`)
admissions_daily_ne$Discharge_Main_Specialty_Desc <- as.character(admissions_daily_ne$Discharge_Main_Specialty_Desc)

admissions_daily_ne$diagnosis_1[is.na(admissions_daily_ne$diagnosis_1)] <- "To Be Coded"
admissions_daily_ne$`ICD10 Short`[is.na(admissions_daily_ne$`ICD10 Short`)] <- "To Be Coded"
...
admissions_daily_ne$`Discharge Date`[is.na(admissions_daily_ne$`Discharge Date`)] <- as.Date("2099-01-01")
...
admissions_daily_ne$`Admission Quarter` <- as.numeric(admissions_daily_ne$`Admission Quarter`)
...
admissions_daily_ne$`Discharge Quarter` <- as.numeric(admissions_daily_ne$`Discharge Quarter`)
...
foreach(i = other_list) %do% {
  admissions_daily_ne$Discharge_Main_Specialty_Desc <-
    ifelse(
      admissions_daily_ne$Discharge_Main_Specialty_Desc == i,
      "OTHER",
      admissions_daily_ne$Discharge_Main_Specialty_Desc
    )
}
admissions_daily_ne$Discharge_Main_Specialty_Desc <- ifelse(admissions_daily_ne$Discharge_Main_Specialty_Desc == "CLINICAL ONCOLOGY", "ONCOLOGY", admissions_daily_ne$Discharge_Main_Specialty_Desc)
admissions_daily_ne$Discharge_Main_Specialty_Desc <- ifelse(admissions_daily_ne$Discharge_Main_Specialty_Desc == "MEDICAL ONCOLOGY", "ONCOLOGY", admissions_daily_ne$Discharge_Main_Specialty_Desc)

Specialty_list <- unique(admissions_daily_ne$Discharge_Main_Specialty_Desc)
...
inflow <- admissions_daily_ne_subset %>%
...
######################################################################################
outflow <- admissions_daily_ne_subset %>%
...
total_ts_ne <- cbind(time_and_total, spec_occupancy)

total_ts_ne <- slice(total_ts_ne, 240:n())
...
total_ts_ne_dept_tbats_list <- total_ts_ne_dept %>%
  select(-new_date) %>%
  group_by(Specialty) %>%
  nest()

total_ts_ne_dept_tbats_7 <- total_ts_ne_dept_tbats_list %>%
  mutate(data_train_7_msts = map(
    .x = data,
    .f = msts,
    start = total_ts_ne_dept$new_date[1],
...
total_ts_ne_dept_tbats_fit <-
  total_ts_ne_dept_tbats_7 %>%
  mutate(fit.tbats = map(data_train_7_msts, tbats))

total_ts_ne_dept_tbats_forecast <-
  total_ts_ne_dept_tbats_fit %>%
  mutate(forecast.tbats = map(fit.tbats, forecast, h = 60))

total_ts_ne_dept_tbats_forecast_tidy <-
  total_ts_ne_dept_tbats_forecast %>%
  mutate(sweep = map(
    forecast.tbats,
    sw_sweep,
...
total_ts_ne_dept_tbats_forecast_tidy$date_value <-
  rep(seq.POSIXt(
    from = total_ts_ne_dept$new_date[1],
    by = "6 hours",
    length.out = nrow(total_ts_ne_dept_tbats_forecast_tidy)/n_groups(total_ts_ne_dept_tbats_forecast_tidy)
  ), times = n_groups(total_ts_ne_dept_tbats_forecast_tidy))

total_ts_ne_dept_tbats_forecast_tidy$`Occupied Beds`[total_ts_ne_dept_tbats_forecast_tidy$`Occupied Beds` <
                                                    0] <-
  0
total_ts_ne_dept_tbats_forecast_tidy$lo.80[total_ts_ne_dept_tbats_forecast_tidy$lo.80 <
                                          0] <- 0
total_ts_ne_dept_tbats_forecast_tidy$lo.95 [total_ts_ne_dept_tbats_forecast_tidy$lo.95 <
                                           0] <- 0

total_ts_ne_dept_tbats_forecast_export <- total_ts_ne_dept_tbats_forecast_tidy %>%
  select(date_value, Specialty, key, `Occupied Beds`, lo.80, lo.95, hi.80, hi.95)
  
write.csv(total_ts_ne_dept_tbats_forecast_export, "ne_6h_tbats.csv")
