...
admissions_daily$Discharge_Main_Specialty_Desc <- ifelse(admissions_daily$`ICD10 Short` == "I21", admissions_daily$`ICD10 Short`, admissions_daily$Discharge_Main_Specialty_Desc)
admissions_daily <- admissions_daily %>% 
...
in_out_depts$CARDIO <- in_out_depts$in_CARDIOLOGY + in_out_depts$out_CARDIOLOGY
in_out_depts$sum_CARDIO <- cumsum(in_out_depts$CARDIO)

in_out_depts$AE <- in_out_depts$`in_ACCIDENT AND EMERGENCY` + in_out_depts$`out_ACCIDENT AND EMERGENCY`
in_out_depts$sum_AE <- cumsum(in_out_depts$AE)

in_out_depts$ACUTE_MI <- in_out_depts$`in_ACUTE MI` + in_out_depts$`out_ACUTE MI`
...
ggplot(in_out_depts) +
  ...
  xlab("Date") +
  ylab("Occupied Beds") +
  scale_x_datetime(date_breaks = "4 months", date_labels = "%Y-%m", limit=c(as.POSIXct("2018-01-01 00:00", tz="UTC"),as.POSIXct("2020-05-18 12:00", tz="UTC"))) +
  geom_line(aes(x = new_date, y = sum_COVID, colour= "U071 Code Patients")) +
  scale_color_manual(values = c(
    'Acute MI' = 'forestgreen',
    'U071 Code Patients' = 'darkorchid3')) +
  labs(color = 'Occupancy Type') +
...
total_forecast_tidy %>%
  ...
  ) + # geom ribbons removed because difficult to see
  geom_line() +
  labs(
