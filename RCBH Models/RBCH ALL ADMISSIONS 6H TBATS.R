...
admissions_daily <- admissions_daily %>%
  filter(
    !is.na(`Admission Date`),!is.na(`Admission Hour`),!is.na(Discharge_Main_Specialty_Desc)
  )

admissions_daily$diagnosis_1 <-
  as.character(admissions_daily$diagnosis_1)
admissions_daily$`ICD10 Short` <-
  as.character(admissions_daily$`ICD10 Short`)
admissions_daily$Discharge_Main_Specialty_Desc <-
  as.character(admissions_daily$Discharge_Main_Specialty_Desc)

admissions_daily$diagnosis_1[is.na(admissions_daily$diagnosis_1)] <-
  "To Be Coded"
admissions_daily$`ICD10 Short`[is.na(admissions_daily$`ICD10 Short`)] <-
  "To Be Coded"


##fixing data for patients currently in hospital
admissions_daily$`Discharge Hour`[is.na(admissions_daily$`Discharge Hour`)] <-
  14
admissions_daily$`Discharge Date` <-
  as.Date(admissions_daily$`Discharge Date`)
admissions_daily$`Discharge Date`[is.na(admissions_daily$`Discharge Date`)] <-
  as.Date("2050-01-01")
...
admissions_daily$Discharge_Main_Specialty_Desc <-
  ifelse(
    admissions_daily$`ICD10 Short` == "I21",
    admissions_daily$`ICD10 Short`,
    admissions_daily$Discharge_Main_Specialty_Desc
  )
admissions_daily <- admissions_daily %>%
  mutate(Discharge_Main_Specialty_Desc = recode(Discharge_Main_Specialty_Desc, I21 = "ACUTE MI"))


admissions_daily$Discharge_Main_Specialty_Desc <-
  ifelse(
    admissions_daily$diagnosis_1 == "U071",
    admissions_daily$diagnosis_1,
    admissions_daily$Discharge_Main_Specialty_Desc
  )
admissions_daily <- admissions_daily %>%
  mutate(Discharge_Main_Specialty_Desc = recode(Discharge_Main_Specialty_Desc, U071 = "COVID"))
admissions_daily$`Admission Date` <-
  as.Date(admissions_daily$`Admission Date`)
admissions_daily$`Discharge Date` <-
  as.Date(admissions_daily$`Discharge Date`)

admissions_daily$`Admission Quarter` <-
  ifelse(
    admissions_daily$`Admission Hour` <= 5,
    1 ,
    ifelse(
      admissions_daily$`Admission Hour` > 5 &
        admissions_daily$`Admission Hour` <= 11,
      2,
      ifelse(
        admissions_daily$`Admission Hour` > 11 &
          admissions_daily$`Admission Hour` <= 17,
        3,
        4
      )
    )
...
admissions_daily$`Discharge Quarter` <-
  ifelse(
    admissions_daily$`Discharge Hour` <= 5,
    1 ,
    ifelse(
      admissions_daily$`Discharge Hour` > 5 &
        admissions_daily$`Discharge Hour` <= 11,
      2,
      ifelse(
        admissions_daily$`Discharge Hour` > 11 &
          admissions_daily$`Discharge Hour` <= 17,
        3,
        4
      )
    )
...
inflow <-
  subset(inflow, select = -c(`Discharge Date`, `Discharge Quarter`))
...
inflow_wide <-
  spread(inflow_spec, Discharge_Main_Specialty_Desc, Count)
...
inflow_wide <-
  inflow_wide %>% dplyr::rename_all(function(x)
    paste0("in_", x))
...
outflow <- admissions_daily_subset %>%
  group_by(`Admission Date`)
outflow <-
  subset(outflow, select = -c(`Admission Date`, `Admission Quarter`))
...
outflow_wide <-
  spread(outflow_spec, Discharge_Main_Specialty_Desc, Count)
outflow_wide[is.na(outflow_wide)] <- 0
outflow_wide$total <- rowSums(outflow_wide[, 3:ncol(outflow_wide)])
outflow_wide[is.na(outflow_wide)] <- 0
outflow_wide[, 3:ncol(outflow_wide)] <-
  lapply(outflow_wide[, 3:ncol(outflow_wide)], function(x)
    - 1 * x)

outflow_wide <-
  outflow_wide %>% dplyr::rename_all(function(x)
    paste0("out_", x))
...
in_out_depts$MONTH <-
  lubridate::month(in_out_depts$Date_value, label = TRUE, abbr = TRUE)
in_out_depts$MONTH <- factor(in_out_depts$MONTH, ordered = FALSE)
in_out_depts$Day_quarter <-
  recode(
    in_out_depts$Day_quarter ,
    `1` = "12PM-6AM",
    `2` = "6AM-12AM",
...
in_out_depts$total <- in_out_depts$in_total + in_out_depts$out_total
in_out_depts$running_total <- cumsum(in_out_depts$total)

in_out_depts$COVID <- in_out_depts$in_COVID + in_out_depts$out_COVID
in_out_depts$sum_COVID <- cumsum(in_out_depts$COVID)

in_out_depts$CARDIO <-
  in_out_depts$in_CARDIOLOGY + in_out_depts$out_CARDIOLOGY
in_out_depts$sum_CARDIO <- cumsum(in_out_depts$CARDIO)

in_out_depts$AE <-
  in_out_depts$`in_ACCIDENT AND EMERGENCY` + in_out_depts$`out_ACCIDENT AND EMERGENCY`
in_out_depts$sum_AE <- cumsum(in_out_depts$AE)

in_out_depts$ACUTE_MI <-
  in_out_depts$`in_ACUTE MI` + in_out_depts$`out_ACUTE MI`
in_out_depts$sum_ACUTE_MI <- cumsum(in_out_depts$ACUTE_MI)
...
in_out_depts$new_date <-
  seq.POSIXt(
    from = as.POSIXct(in_out_depts$Date_value[1]),
...
covid_v_all <- ggplot(in_out_depts) +
  geom_line(aes(y = running_total, x = new_date, colour = "All Occupancy")) +
  xlab("Date") +
  ylab("Occupied Beds") +
  scale_x_datetime(
    date_breaks = "4 weeks",
...
ggplot(in_out_depts) +
  ...
  geom_hline(yintercept = 12,
             linetype = "dashed",
             color = "red3") +
  scale_color_manual(values = c('A+E Occupancy' = 'forestgreen',
                                'U071 Code Patients' = 'darkorchid3')) +
  labs(color = 'Occupancy Type')

ggplot(in_out_depts) +
  geom_line(aes(y = sum_ACUTE_MI, x = new_date, colour = "Acute MI")) +
  xlab("Date") +
  ylab("Occupied Beds") +
  scale_x_datetime(
    ...
    date_labels = "%Y-%m",
    limit = c(
      as.POSIXct("2018-01-01 00:00", tz = "UTC"),
      as.POSIXct("2020-05-18 12:00", tz = "UTC")
    )
  ) +
  geom_line(aes(x = new_date, y = sum_COVID, colour = "U071 Code Patients")) +
  scale_color_manual(values = c('Acute MI' = 'forestgreen',
                                'U071 Code Patients' = 'darkorchid3')) +
  labs(color = 'Occupancy Type') +
  geom_smooth(aes(y = sum_ACUTE_MI, x = new_date, method = "loess"))


in_out_depts_oh <-
  one_hot(as.data.table(in_out_depts),
          cols = "auto",
          dropCols = TRUE)
...
total_ts <-
  subset(in_out_depts_oh, select = c(new_date, running_total))
...
total_msts <-
  msts(total_ts_2, seasonal.periods = c(28, 1461)) #weekly and yearly frequencies of 6 hours
...
total_forecast_tidy %>%
  ggplot(aes(x = date_value ,
             y = running_total,
             color = key)) + # geom ribbons removed because difficult to see
  geom_line() +
  labs(
    title = "RCBH All Classification Admissions Forecast, 6 Hourly Definition",
  ...
  scale_x_datetime(breaks = "1 month",
                   limits = c(
                     as.POSIXct("2020-01-01 00:00", tz = "UTC"),
                     as.POSIXct("2020-07-01 00:00", tz = "UTC")
                   ))
