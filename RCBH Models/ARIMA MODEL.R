...
midnight_beds_train <-
  midnight_beds %>% filter(Date_Value < "2020-03-01")
...
midnight_beds_train_list_forecast <-
  midnight_beds_train_list_ts_arima %>%
  mutate(forecast.arima = map(fit.arima, forecast, h = 120))
...
midnight_beds_train_list_forecast_tidy$`Occupied Beds`[midnight_beds_train_list_forecast_tidy$`Occupied Beds` <
                                                         0] <- 0
midnight_beds_train_list_forecast_tidy$lo.80[midnight_beds_train_list_forecast_tidy$lo.80 <
                                               0] <- 0
midnight_beds_train_list_forecast_tidy$lo.95 [midnight_beds_train_list_forecast_tidy$lo.95 <
                                                0] <- 0
...
midnight_beds_list_ts_overlay_arima <-
  midnight_beds_list_ts_overlay %>%
  mutate(fit.arima = map(data_train.ts, auto.arima))
midnight_beds_list_for_overlay_forecast <-
  midnight_beds_list_ts_overlay_arima %>%
...
midnight_beds_list_overlay_forecast_tidy$`Occupied Beds`[midnight_beds_list_overlay_forecast_tidy$`Occupied Beds` <
                                                           0] <- 0
midnight_beds_list_overlay_forecast_tidy$lo.80[midnight_beds_list_overlay_forecast_tidy$lo.80 <
                                                 0] <- 0
midnight_beds_list_overlay_forecast_tidy$lo.95 [midnight_beds_list_overlay_forecast_tidy$lo.95 <
                                                  0] <- 0
...
ggplot(
  ...
  geom_ribbon(
    data = midnight_beds_train_list_forecast_tidy,
    aes(ymin = lo.95, ymax = hi.95),
    fill = "#D5DBFF",
    color = NA,
    size = 0
